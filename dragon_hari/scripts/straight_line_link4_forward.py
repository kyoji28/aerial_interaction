...
class StraightLineLink4Forward:
...
def main():
    rospy.init_node("straight_line_link4_forward")

    node = StraightLineLink4Forward()

    rospy.loginfo(
        "straight_line_link4_forward node started."
    )
    ...
    completed = node.run_link4_forward(
        goal_x,
        goal_y,
    )
    ...
    if not completed:
        rospy.loginfo(
            "Link4-forward demo did not complete."
        )

    rospy.loginfo(
        "straight_line_link4_forward terminated."
    )
